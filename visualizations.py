...
class VisualizationEngine:
    def generate_plots(df):
        try:
            ...
            bars1 = plt.bar(x - width / 2, churn_no, width, label='Churn = No', color='g')
            bars2 = plt.bar(x + width / 2, churn_yes, width, label='Churn = Yes', color='y')
            for bar in bars1:
                value = bar.get_height()
                percent = (value / total_customers) * 100
                plt.text(bar.get_x() + bar.get_width() / 2, value / 2, f"{int(value)}\n{percent:.1f}%", ha='center', va='center', color='white', fontsize=9)
            for bar in bars2:
                value = bar.get_height()
                percent = (value / total_customers) * 100
                plt.text(bar.get_x() + bar.get_width() / 2, value / 2, f"{int(value)}\n{percent:.1f}%", ha='center', va='center', color='white', fontsize=9)
            ...
            plt.text(0.98, 0.95, 'Senior Citizen\n0 = Non-Senior\n1 = Senior', transform=plt.gca().transAxes, ha='right', va='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.85))
